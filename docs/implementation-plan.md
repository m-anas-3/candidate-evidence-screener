...
| Automated tests  | **Installed and configured**                                           | Vitest covers report authority, scoring, prompt safety, proposal signals, and portfolio score exclusion; live ownership acceptance remains |
...
| -------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------- | ------------- | -------------------------------------------------------------------------------------------------- |
| Screen freelance candidates using job, resume, and proposal evidence | Persist evidence and present one report; portfolio is optional manual context                         | 2, 4, 5, 8    | End-to-end manual and synthetic candidate acceptance tests                                         |
...
| Provide custom tools                                                 | Authorized context load, proposal-specificity assessment, and report persistence                      | 6             | Unit tests for schemas, tool guards, and tool-result binding                                       |
...
**Status:** implemented. The server-only `createDeepAgent()` harness uses the locked model default and medium reasoning, a versioned recruiter prompt, strict evidence/report validation, three ownership-bound product tools, and an in-memory backend with non-product tools hidden. Proposal-tool results are authoritative at persistence. Portfolios are not fetched or scored. Live model integration remains part of Milestone 7.

**Depends on:** Milestones 1–2 and 5; Deep Agents/LangChain/OpenAI packages and model configuration.

Create a server-only `createDeepAgent()` harness using `gpt-5.5` with medium reasoning initially, a versioned recruiter prompt, strict report schema, and the three locked custom tools. Encode the locked 55/30/15 scoring weights, recommendation bands, zero-for-unsupported rule, and 79-point cap when a must-have lacks evidence. Enforce ownership in context loading and saving. Do not fetch or score portfolio content.
...
**Status:** implemented locally. An authenticated one-click action creates a fictional job and candidate with synthetic proposal and resume evidence already ready for analysis, plus a same-application portfolio fixture for manual review. Manual intake remains available. End-to-end timing and Vercel Fluid Compute verification remain pending deployment.
...
**Status:** partially complete. Vitest is configured with focused no-network coverage for authoritative tool-result binding, portfolio score exclusion, must-have repair and caps, recommendation boundaries, proposal signals, contradictory report data, and chat prompt safety. Lint and typecheck pass locally. Live authentication/RLS cross-owner tests, analysis transition integration tests, a production build, and deployed smoke/security checks remain.
...
- **Scoring:** 55 points for job requirements/skills, 30 for relevant experience evidence, and 15 for proposal specificity. Portfolio review is manual and does not affect the score. Strong Fit is 80–100, Possible Fit is 60–79, and Weak Fit is 0–59. Unsupported criteria score zero, and a missing must-have caps the score at 79.
