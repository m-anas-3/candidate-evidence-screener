    ...
    <div className="mx-auto w-full max-w-3xl space-y-6">
      ...
      <div className="space-y-1.5">
        ...
        <p className="text-sm text-muted-foreground">
          Provide candidate details and resume to screen them against the role criteria.
        </p>
      </div>
          ...
          <Card className="border-border/50 shadow-sm bg-muted/10">
            ...
            <CardContent className="pt-4 space-y-3">
              <div>
                ...
                <p className="text-2xs text-muted-foreground mt-0.5">Job criteria identifier: {job.id}</p>
              </div>
              {job.must_have_skills.length > 0 && (
                <div className="space-y-1.5 pt-2">
                  <span className="text-2xs font-semibold text-muted-foreground">Required Skills:</span>
                  <div className="flex flex-wrap gap-1.5">
                    {job.must_have_skills.slice(0, 5).map((skill) => (
                      <Badge key={skill} variant="secondary" className="text-2xs font-normal">
                        {skill}
                      </Badge>
                    ))}
          ...
          <Card className="border-border/50 shadow-sm">
            ...
            <CardContent className="pt-4 space-y-4">
              <div className="flex gap-2.5">
                <span className="flex size-5 shrink-0 items-center justify-center rounded-full bg-primary/10 text-primary text-2xs font-bold mt-0.5">
                  1
                </span>
                <p className="text-xs text-muted-foreground leading-relaxed">
                  Enter candidate's name and paste their original proposal.
                </p>
              </div>
              <div className="flex gap-2.5">
                <span className="flex size-5 shrink-0 items-center justify-center rounded-full bg-primary/10 text-primary text-2xs font-bold mt-0.5">
              ...
              <div className="flex gap-2.5">
                ...
                <p className="text-xs text-muted-foreground leading-relaxed">
                  Submit to upload. You'll be redirected to extract the resume text and run fit analysis.
                </p>
              </div>
            </CardContent>
          </Card>
